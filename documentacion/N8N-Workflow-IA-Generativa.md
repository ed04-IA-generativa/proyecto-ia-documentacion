...
3. Se corrigieron las credenciales del nodo `Login`: el usuario de prueba original (`devgd02`) dejó de funcionar porque `ApiKnowledge` apunta ahora a la base `test_cuenta_corriente_20250828` (antes apuntaba a `DLBENDICION_...`) — se actualizó a un usuario válido en esa base.
4. Se ejecutó el workflow completo (`execute_workflow`, modo manual) contra los servicios reales corriendo en local (ApiKnowledge por `dotnet run`, Postgres y n8n en Docker) y se confirmó `status: success` en las 7 nodos, incluyendo `Consultar Ventas` trayendo 10 filas reales de ventas (producto, cliente, cantidad, monto, canal, coordenadas GPS, etc.).
...
4. **Requisitos para que corra de verdad**:
   - ApiKnowledge corriendo localmente en `https://localhost:7225` (perfil `https` de `dotnet run`), accesible desde el contenedor de n8n como `host.docker.internal:7225`.
